/* ============ Capa de datos (localStorage) ============
   Compatible con los datos y respaldos de StreamGest 1.x:
   misma clave 'sg_data_v1' y mismas migraciones. */
"use strict";
...
function dbDefault(){
  return {
    ...
    registros: [],        // {id, clienteId, fecha, nota, items:[...], pagos:[...], esperado:{}, pagado:{}, saldo:{}}
    recargas: [],         // {id, cuentaId, fecha, pagos:[...], total:{}, dias, vence}
    templates: {
    ...
    config: {
      usuario: '',         // nombre de quien usa la app
      tema: 'auto',        // 'auto' | 'dark' | 'light'
      avisoCliente: 7,     // días de anticipación para "próximos vencimientos"
      avisoRecarga: 5,     // días de anticipación para recargas por vencer
      ultimoRespaldo: null // fecha ISO del último respaldo exportado
    }
  };
}
...
function dbMigrate(){
  ...
  if (!DB.config.tema) DB.config.tema = localStorage.getItem('sg_tema') || 'auto';
  if (!(Number(DB.config.avisoCliente) >= 1)) DB.config.avisoCliente = 7;
  if (!(Number(DB.config.avisoRecarga) >= 1)) DB.config.avisoRecarga = 5;
  if (DB.config.ultimoRespaldo === undefined) DB.config.ultimoRespaldo = null;
  ...
  const monedaDeMetodoLegacy = (met) => {
    ...
    if (met.requiereTasa) return ensureMoneda(met.nombre, met.nombre);
    return usdId;
  };

  // ---- 2. Métodos: {requiereTasa, modoTasa} -> {monedaId} ----
  (DB.metodos || []).forEach(m => {
    if (!m.monedaId) m.monedaId = monedaDeMetodoLegacy(m);
    delete m.requiereTasa; delete m.modoTasa;
  });

  // ---- 3. Plataformas: precioUsd -> precio ----
  ...
  // ---- 4. Reconstruir suscripciones desde el historial ----
  ...
  // ---- 5. Registros: modelo antiguo -> por-moneda ----
  ...
  // ---- 6. Recargas: modelo antiguo -> por-moneda ----
...
function restarMapas(a, b){
  ...
  keys.forEach(k => { out[k] = round2((a[k] || 0) - (b[k] || 0)); });
  return out;
}
...
/* "$12.00" para USD, "45.000 COP" para el resto */
...
/* Mapa {monedaId: monto} como "45.000 COP · $10.00" */
...
/* Saldo acumulado del cliente por moneda (negativo = debe) */
...
function clienteEstado(clienteId){
  ...
  if (!subs.length) return { label: 'Sin planes', cls: 'gray', key: 'sinplanes' };
  const conVence = subs.filter(s => s.vence);
  const algunoVencido = conVence.some(s => daysLeft(s.vence) < 0);
  if (algunoVencido) return { label: 'Vencido', cls: 'red', key: 'vencido' };
  ...
  if (debe) {
    ...
    return { label: 'Debe ' + fmtMapa(deudas), cls: 'red', key: 'debe' };
  }
  if (subs.some(s => !s.vence)) return { label: 'Sin activar', cls: 'amber', key: 'sinactivar' };
  const proximo = conVence.some(s => daysLeft(s.vence) <= 1);
  if (proximo) return { label: 'Por vencer', cls: 'amber', key: 'porvencer' };
  return { label: 'Al día', cls: 'green', key: 'aldia' };
}

/* Total por cobrar: suma de las deudas de todos los clientes, por moneda */
function totalPorCobrar(){
  const out = {};
  DB.clientes.forEach(c => {
    const saldo = clienteSaldo(c.id);
    Object.keys(saldo).forEach(k => { if (saldo[k] < -0.009) out[k] = round2((out[k] || 0) - saldo[k]); });
  });
  return out;
}
...
/* Ingresos y egresos de una moneda en los últimos n meses:
   [{key:'2026-07', label:'Jul', ing, egr}] */
function serieMensual(monedaId, n){
  n = n || 6;
  ...
  for (let i = n - 1; i >= 0; i--) {
    ...
    meses.push({ key, label: d.toLocaleDateString('es-VE', { month:'short' }), ing: 0, egr: 0 });
  }
  const idx = {};
  meses.forEach(m => { idx[m.key] = m; });
  DB.registros.forEach(r => {
    const m = idx[String(r.fecha||'').slice(0,7)];
    if (m) m.ing = round2(m.ing + ((r.pagado || {})[monedaId] || 0));
  });
  DB.recargas.forEach(r => {
    const m = idx[String(r.fecha||'').slice(0,7)];
    if (m) m.egr = round2(m.egr + ((r.total || {})[monedaId] || 0));
  });
  return meses;
}

/* Monedas con algún movimiento registrado (para el selector del gráfico) */
function monedasConMovimiento(){
  const set = new Set();
  DB.registros.forEach(r => Object.keys(r.pagado || {}).forEach(k => set.add(k)));
  DB.recargas.forEach(r => Object.keys(r.total || {}).forEach(k => set.add(k)));
  return Array.from(set).filter(k => getMoneda(k))
    .sort((a,b) => monedaCodigo(a).localeCompare(monedaCodigo(b)));
}
...
/* Renovaciones que vienen en los próximos `dias` días (incluye hoy) */
function proximasRenovaciones(dias){
  const out = [];
  DB.clientes.forEach(c => {
    clienteSuscripciones(c.id).forEach(s => {
      if (!s.vence) return;
      const d = daysLeft(s.vence);
      if (d >= 0 && d <= dias) out.push({ cliente: c, plan: s, dias: d });
    });
  });
  return out.sort((a,b) => a.dias - b.dias);
}

/* Cuántos clientes usan una cuenta propia */
function cuentaClientes(cuentaId){
  return DB.clientes.filter(c => (c.suscripciones||[]).some(s => s.cuentaId === cuentaId)).length;
}

/* ============ Alertas ============ */
function calcAlertas(){
  const avisoCli = Number(DB.config.avisoCliente) || 7;
  const avisoRec = Number(DB.config.avisoRecarga) || 5;
  const cliVencidos = [];
  const cliPorVencer = [];
  const cliProximos = [];   // informativo, no cuenta en el badge
  DB.clientes.forEach(c => {
    clienteSuscripciones(c.id).forEach(s => {
      if (!s.vence) return;
      const d = daysLeft(s.vence);
      ...
      else if (d <= avisoCli) cliProximos.push({ cliente: c, plan: s, dias: d });
    });
  });
  const recPorVencer = [];
  DB.cuentas.forEach(ct => {
    ...
    if (d <= avisoRec) recPorVencer.push({ cuenta: ct, recarga: ultima, dias: d });
  });
  ...
  return { cliVencidos, cliPorVencer, cliProximos, recPorVencer, avisoCli, avisoRec };
}
