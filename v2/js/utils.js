...
/* Números compactos para el gráfico: 1.2k, 3.4M */
...
const Theme = {
  KEY: 'sg_tema',
  get(){
    if (typeof DB !== 'undefined' && DB && DB.config && DB.config.tema) return DB.config.tema;
    return localStorage.getItem(Theme.KEY) || 'auto';
  },
  apply(){
    ...
    if (meta) meta.content = dark ? '#0b0e14' : '#eef1f6';
  },
  set(t){
    localStorage.setItem(Theme.KEY, t);
    if (typeof DB !== 'undefined' && DB && DB.config) { DB.config.tema = t; dbSave(); }
    Theme.apply();
  }
};
try {
...
/* ---- Confirmación bonita (reemplaza a confirm()) ----
   Uso: if (!(await confirmar('¿Eliminar esto?', {danger:true, ok:'Eliminar'}))) return; */
  ...
  return new Promise(resolve => {
    ...
    root.innerHTML = `
      <div class="modal confirm">
        <div class="confirm-icon ${opts.danger ? 'danger' : ''}">${opts.danger ? I.alert : I.info}</div>
        <p>${msg}</p>
        <div class="modal-actions">
          <button class="btn ghost" id="cfm-no">${esc(opts.no || 'Cancelar')}</button>
          <button class="btn ${opts.danger ? 'danger-solid' : ''}" id="cfm-si">${esc(opts.ok || 'Confirmar')}</button>
        </div>
      </div>`;
    const done = (v) => { root.innerHTML = ''; resolve(v); };
    document.getElementById('cfm-no').onclick = () => done(false);
    document.getElementById('cfm-si').onclick = () => done(true);
...
/* CSV con BOM para que Excel lo abra bien (acentos incluidos) */
...
function headHTML(o){
  return `
  <div class="page-head">
    ${o.back ? `<button class="icon-btn back" aria-label="Volver" onclick="App.nav('${o.back}')">${I.back}</button>` : ''}
    <div style="flex:1;min-width:0"><h1>${o.title}</h1>${o.sub ? `<div class="sub">${o.sub}</div>` : ''}</div>
...
const I = {
  ...
  auto:   '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 2a10 10 0 0 0 0 20z" fill="currentColor" stroke="none"/></svg>',
  ...
  trendUp:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M23 6l-9.5 9.5-5-5L1 18"/><path d="M17 6h6v6"/></svg>',
  trendDown:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M23 18l-9.5-9.5-5 5L1 6"/><path d="M17 18h6v-6"/></svg>'
};
